...
Base: consolidar.md v2.2.2 + candidato v2.3.0-rc.3 + hardening final de validação objetiva
---
...
3. persistir UMA cópia;
4. obter FILE_ID ou identificador estável equivalente retornado pelo provedor;
5. não alterar o artefato integral para inserir resultados posteriores da persistência.

Nome recomendado: <PROJECT_ID>__CONSOLIDADO__CONTINUIDADE__<AAAAMMDD-HHMM>__v<versao>.

Registrar na transação posterior, quando disponíveis: RUN_ID, PROJECT_ID, provider, nome, FILE_ID, path lógico, link estável e versão de /consolidar.
...
Se qualquer item falhar: MODO DE ENTREGA: INTEGRAL.
...
Ordem preferencial: CONTINUITY_REF conhecido; FILE_ID conhecido; registro governante do projeto; descoberta restrita à residência confirmada; busca nominal somente como fallback.

Havendo múltiplos consolidados, não presumir que o mais recente é automaticamente o mais autoritativo. Considerar PROJECT_ID, escopo, RUN_ID, data, artefatos governantes, estado e relevância.
